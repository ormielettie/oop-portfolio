...
class MemberCard(tk.Frame):
    """
    Inherits from tk.Frame. This class acts as a template 
    to display a specific student's profile information.
    """
    ...
    def load_profile_image(self):
        """Dynamically crops and scales the photo."""
        try:
            if self.photo_path:
...
class PortfolioApp:
    def __init__(self, root):
        ...
        self.root.geometry("590x510") # Slightly taller to clear room for the new button strip cleanly
        self.root.configure(bg="#FDF1ED")
        self.root.resizable(True, True)
        
        # Team Dictionary List Setup
        self.members_data = {
            "AEROLD AFABLE": {"role": "TESTER", "photo": "aerold.jpg"},
            "CHRISTIAN BERGOLA": {"role": "TESTER", "photo": "christian.jpg"},
            "EUNICE BLANCO": {"role": "DESIGNER", "photo": "eunice.jpg"},
            "ZAINA CUASAY": {"role": "DOCUMENTATION", "photo": "zaina.jpg"},
            "IRISH DE GUZMAN": {"role": "DEVELOPER", "photo": "irish.jpg"},
            "LIRAH MOLLEDA": {"role": "DOCUMENTATION", "photo": "lirah.jpg"},
            "JANA MIKAELA SAET": {"role": "DEVELOPER", "photo": "jana.jpg"},
            "ANNA SHARLENE TORIO": {"role": "PORTFOLIO DEVELOPER", "photo": "anna.jpg"},
            "YAEL TORREGOZA": {"role": "DEVELOPER", "photo": "yael.jpg"},
            "KIV ZEIMER CRUUZ": {"role": "DESIGNER", "photo": "kiv.jpg"}
        }
    ...
    def create_window_menu(self):
        """Creates a fully functional drop-down system menu bar layout."""
    ...
    def setup_layout_containers(self):
        # ------------------------------------------
        # NEW ROW: "GO TO PROFILES" Dropdown Trigger Strip
        # ------------------------------------------
        button_strip = tk.Frame(self.root, bg="#FDF1ED", pady=5)
        button_strip.pack(fill="x", side="top")
        
        self.goto_btn = tk.Button(
            button_strip, 
            text="GO TO PROFILES ▼", 
            font=("Inter Bold", 9, "bold"),
            bg="#FFD000", 
            fg="#7A151A", 
            activebackground="#E6B800",
            activeforeground="#7A151A", 
            bd=0, 
            padx=12, 
            pady=5,
            command=self.show_dropdown_menu
        )
        self.goto_btn.pack(side="left", padx=20, pady=5)
    ...
    def show_dropdown_menu(self):
        """Generates and maps the sub-menu context directly below the button."""
        dropdown = tk.Menu(self.root, tearoff=0, font=("Inter", 9), bg="#FFFFFF", fg="#212529", activebackground="#7A151A", activeforeground="#FFFFFF")
        dropdown.add_command(label="View Portfolios", command=lambda: messagebox.showinfo("LMS Notice", "Portfolio module active!"))
        dropdown.add_separator()
        dropdown.add_command(label="Exit Component", command=self.root.quit)
        
        # Find position immediately beneath the button widget geometry rules
        x = self.goto_btn.winfo_rootx()
        y = self.goto_btn.winfo_rooty() + self.goto_btn.winfo_height()
        dropdown.post(x, y)

    def update_view(self):
        """Clears old widgets and renders the chosen student's portfolio view panel."""
